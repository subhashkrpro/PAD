...
class OverlayUI:
    ...
    def draw_verdict(frame, verdict: str, confidence: float, score: float):
        ...
        cv2.rectangle(frame, (0, 0), (w, 100), COLOR_BG_DARK, -1)
        cv2.rectangle(frame, (0, 96), (w, 100), color, -1)
        ...
        cv2.putText(
            frame, sub_text,
            (20, 78), cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_WHITE, 2,
        )
        ...
        cv2.putText(
            frame, conf_text,
            (w - 300, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_WHITE, 2,
        )
    ...
    def draw_small_indicator(frame, text: str):
        ...
        cv2.putText(
            frame, text,
            (w - 160, h - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLOR_YELLOW, 1,
        )
    ...
    def draw_score_bars(frame, scores: dict[str, float], y_start: int = 110):
        ...
        for i, (name, val) in enumerate(scores.items()):
            ...
            cv2.putText(
                frame, name,
                (label_x, y + 3), cv2.FONT_HERSHEY_SIMPLEX, 0.45, COLOR_WHITE, 1,
            )
            ...
            cv2.putText(
                frame, f"{val:.2f}",
                (x_start + bar_w + 5, y + 3), cv2.FONT_HERSHEY_SIMPLEX, 0.4, COLOR_WHITE, 1,
            )
    ...
    def draw_help(frame):
        ...
        h, w = frame.shape[:2]
        cv2.rectangle(frame, (0, h - 35), (w, h), COLOR_BG_DARK, -1)
    ...
    def draw_fps(frame, fps: float):
        ...
        cv2.putText(
            frame, f"FPS: {fps:.0f}",
            (15, 95), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLOR_YELLOW, 1,
        )
    ...
    def draw_analyzing(frame):
        ...
        h, w = frame.shape[:2]
        cv2.rectangle(frame, (0, 0), (w, 70), COLOR_BG_DARK, -1)
        cv2.putText(
            frame, "ANALYZING...",
            (15, 45), cv2.FONT_HERSHEY_SIMPLEX, 1.2, COLOR_YELLOW, 2,
        )

    @staticmethod
    def draw_status(frame, text: str, color=COLOR_WHITE):
